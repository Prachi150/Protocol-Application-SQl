# ==============================================================================
# app.py — IoConnect Protocol Adapter Main Entry Point (Python example)
#
# This file is IDENTICAL across the OPC UA, Modbus, and S7 adapters.
# Copy it verbatim for any new Python protocol adapter. Only three lines
# need changing — search for "# REPLACE:" to find them.
#
# What this file does:
...
#   4. Instantiates one protocol client per server entry in the poll config
...
from opc import OPCUAClient                          # REPLACE: change to your protocol module and class
import signal
...
def poll(config : list, protocol_obj : OPCUAClient, post_obj : post_handler, stop_event : threading.Event, server_url : str):
    # REPLACE: OPCUAClient above — change to your protocol client class name in the type hint
                        ...
                        for packet in rate_config["packets"]:
                            ...
                            if tags is not None:
                                ...
                                for tag in tags:
                                    if tag["status"] == 1:
                                        success = True
                                        data_packet = data_packet + tag["tags"]
                                        good_count += len(tag["tags"])
                                    else:
                                        stat = 2
                                        bad_count += 1
                                        logger.warning("Tag read returned error", extra={"ctx": {
                                            "server": server_url,
                                            "device_id": packet["device_id"],
                                            "error": tag["error"]
                                        }})
                                ...
                                logger.warning("Empty tag array received from protocol read", extra={"ctx": {
                                    "server": server_url,
                                    "device_id": packet["device_id"]
                                }})
                            ...
                            if success:
                                data_packet = data_packet + [
                                    {"tag": "RSSI", "value": 22},
                                    {"tag": "Status", "value": stat}
                                ]
                            else:
                                data_packet = [{"tag": "RSSI", "value": 22}, {"tag": "Status", "value": 0}]
                            ...
                            logger.info("Payload queued for posting", extra={"ctx": {
                                "server": server_url,
                                "device_id": packet["device_id"],
                                "tag_count": len(data_packet),
                                "tags": data_packet
                            }})
...
def main():
    ...
    if os_broker_address:
        posting_config = {
            ...
            "client_id": os.environ.get("REDPANDA_KAFKA_CLIENT_ID", "opcua-protocol"),
    ...
    for server_entry in poll_config:
        server_key = server_entry['protocol']['server']
        try:
            protocol_obj = OPCUAClient(server_entry)   # REPLACE: change OPCUAClient to your protocol client class
        except Exception:
